...
-------------
2022-04<br>
4月份利用空余时间学习了解redis，在项目基础之上添加了redis缓存连接池和数据库同步工具<br>
数据记录逻辑：<br>
    写入数据：直接写入mysql<br>
    读入数据：从redis集群中的从机读取<br>
    数据同步：开启新线程，每隔一段时间完成mysql数据到redis主机的同步<br>
...
* 测试前确认已安装MySQL数据库以及安装并启动redis(主机端口6379,从机端口6380,6381)
